...
class TriadQuiz extends Component {
    ...
    render(){
        ...
        return(
            <div>
                <button id="1" className = "quiz-button"onClick={this.handleChange}>{this.state.clicked ? "Quit" : "Triad Quiz"}</button>
                {
                    this.state.clicked  &&
                    <div>

                        {this.state.quizOver &&<div className = "quiz-text"> Quiz Over  Score: {this.state.score}/10, click Quit to exit</div>}
                        {!this.state.quizOver && <div className = "quiz-text">Score: {this.state.score}/10 -- Question #{1+this.state.questionNumber } </div>}
                        <button id="3"className = "quiz-button" onClick={this.handleChange}>Randomize Note Spelling {this.state.randomized ? "    (On)" : "    (Off)"}</button>
                        <button className = "quiz-button" id="4" onClick={this.handleChange}>Seventh Chords{this.state.seventh? "    (On)" : "    (Off)"}</button>
                        <div className = "quiz-text">Which of the following notes compose {this.state.question}</div>
